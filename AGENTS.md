...
Group 8 (Tasks 8.1–8.13) implemented the plan's handlers/renders as written — `crates/tui/src/screens/*.rs` is authoritative; the plan text is stale in four places:
...
- Test counts again: the Task 8.13 gate reports api-client 57 + state 30 + tui 40 (plan claims 52/25/40; only the tui 40 is right). Same rustfmt caveat as Groups 4–7: the plan's screen blocks are not fmt-clean at 100 cols; `cargo fmt` was applied at Task 8.13. The `handle_room_binding_key`/`handle_run_key` `state` params are intentionally unused (plan-literal) — release build has only warnings, not errors.
